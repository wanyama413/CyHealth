class MyApp {
        ...
        countries.map((country)=>{
            ...
            statistics.forEach((element)=>{
                ...
                if (countryy == `${country}`) {
                    ...
                    if (String(newCasess) == "null") newCases.innerText = "0";
                    else newCases.innerText = String(newCasess).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                    //cumulativeDeaths
                    ...
                    if (String(newDeathss) == "null") newDeaths.innerText = "0";
                    else newDeaths.innerText = String(newDeathss).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                    //criticalCases
                    ...
                    if (String(critical) == "null") criticalCases.innerText = "0";
                    else criticalCases.innerText = String(critical).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                    //missing real data
    ...
    Filter() {
        ...
        for(i = 0; i < tr.length; i++){
            ...
            if (td) {
                ...
                if (txtValue.indexOf(capitalize(search)) > -1) tr[i].style.display = "";
    ...
    exportTableToCSV(filename) {
        ...
        for(var i = 0; i < rows.length; i++){
            ...
            for(var j = 0; j < cols.length; j++)row.push(cols[j].textContent);
            console.log(row);
...
class CountryFlag {
    ...
    /** @private */ static getCountryByMapName(keyIndex, code) {
        const mapName = `map-${keyIndex}`;
        CountryFlag.lazyLoadCountryMap(mapName, keyIndex);
        return CountryFlag[mapName].get(code);
    }
    /** @private */ static lazyLoadCountryMap(mapName, keyIndex) {
        CountryFlag.lazyLoadCountryList();
        if (!CountryFlag[mapName]) {
            CountryFlag[mapName] = new Map();
            for (const country of CountryFlag.countryList)CountryFlag[mapName].set(country[keyIndex], country);
        }
    }
    /** @private */ static lazyLoadCountryList() {
        if (!CountryFlag.countryList) CountryFlag.countryList = JSON.parse(CountryFlag.rawCountryList);
    }
}
/* I wanted these below to be either static class members or a top-level module constants,
   but I'm avoiding it for now to maximize browser compatibility */ CountryFlag.IDX_NUMERIC = 0;
